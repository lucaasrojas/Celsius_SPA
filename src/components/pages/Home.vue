<template>
  <div>
    ...
    <div v-if="false" class="row home-news">
      <div class="col image">
        <img
          src="https://i2.wp.com/www.downshiftology.com/wp-content/uploads/2017/05/Chemex-Coffee-Brewing-11.jpg"
          alt=""
        />
      </div>
...
export default {
  ...
  data() {
    return {
      ...
      techs: [
        { icon: "react", redirect: "https://es.reactjs.org/" },
        { icon: "vuejs", redirect:  "https://vuejs.org/"},
        { icon: "github", redirect: "https://github.com/" },
        { icon: "aws", redirect: "https://aws.amazon.com/" },
        { icon: "sass", redirect: "https://sass-lang.com/" },
      ],
      appList: [],
      aboutMe: {
        title: "",
  ...
  methods: {
    ...
    getAppList() {
      const appsMenu = this.dbData.config.menuItems.find(
        (item) => item.title === "apps"
      );
      this.appList = appsMenu.subItems.filter((app) => app.visible && app);
      this.appList = this.appList.map((app) => {
        return { ...app, kindTitle: this.getTraducedText(app.title) };
      });
    },
    updateJumbotron() {
      const langData = this.getTraducedText("home");
      this.$emit("jumbotronData", {
        mainTitle: langData.mainTitle,
        mainDescription: langData.mainDescription,
      });
    },
    updateAboutMe() {
